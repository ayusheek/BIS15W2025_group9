...
1. How many hits and flops are there? Provide a table.
```{r}
bollywood <- bollywood %>%
...
bollywood %>% 
  ...
  summarize(hit_count = n()) %>%
    ungroup()
```
There are 699 hits and 42 flops in total.

2)How many hits and flops are there in the Bollywood dataset? Provide the appropriate plot.
```{r}
ggplot(bollywood %>% 
         mutate(success = case_when(
           hit_flop.x < 7 ~ "flop", 
           hit_flop.x >= 7 ~ "hit"))) +
  geom_bar(aes(x = success, fill = success)) +
  labs(title = "Number of Hits and Flops", 
       x = "Movie Success", 
       y = "Count") +
  scale_fill_manual(values = c("hit" = "green", "flop" = "red")) +
  theme_minimal()
```


3. Compare hits and flops by whether they are remakes or not. Provide a table.
```{r}
bollywood %>%
...
```
There are 660 flops that were not remade, 33 hits were were not remade.Then, there was 39 flops that were remade and 9 hits that were remade.

4. How do the number of hit and flop movies compare for remakes versus non-remakes in the Bollywood dataset? Use the information above to create a plot.
```{r}
bollywood %>%
  filter(!is.na(whether_remake)) %>%
  group_by(whether_remake, success) %>%
...
4. How many hits and flops were there by type genre. Create the appropriate plot.
```{r}
genre_summary <- bollywood %>%
  group_by(genre, hit_flop.x) %>%
  summarize(count = n(), .groups = "drop")
...
```{r}
ggplot(genre_summary, aes(x = genre, y = count)) +
  geom_bar(stat = "identity") +  # Dodge position to show bars side by side
  labs(
    title = "How many Hits and Flops by Genre",
    x = "Genre",
    y = "Number of Movies",
...
success_by_period <- bollywood %>%
  group_by(release_period, hit_flop.x) %>%
  summarize(count = n(), .groups = "drop")
success_by_period

```
On holiday there were 295 hit and flops total released and on normal days, there were 243 hits and flops released. 
...
```{r}
ggplot(actor_summary, aes(x = hit_flop.x, y = count, fill = lead_star)) +
